...
exports.getProductBySlug = (req, res) => { 
    ...
    // Tìm sản phẩm
    const product = productsData.find(p => p.id === productId && p.slug === productSlug);
...
exports.getDifferentProductsById = async (req, res) => {
    try {
        ...
        const randomProducts = fillerdProducts.sort(() => Math.random() - 0.5) // Trộn mảng ngẫu nhiên
...
exports.getSimilarProductById = async (req, res) => {
    try {
        ...
        const fillerdProducts = productsData.filter(p => p.categoryId === categoryId)
        const randomProducts = fillerdProducts.sort(() => Math.random() - 0.5) // Trộn mảng ngẫu nhiên
        .slice(0, 4); // Lấy 4 phần tử đầu tiên
        return res.json(randomProducts);  // Trả về 4 sản phẩm ng��u nhiên của người bán
    }catch(error) {
        console.error("Lỗi khi lấy sản phẩm tương tự", error);
        return res.status(500).json({ message: "Lỗi server khi lấy sản phẩm tương tự", error });
    }
}
