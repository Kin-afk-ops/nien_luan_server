...
exports.getSimilarProductById = async (req, res) => {
  try {
    ...
    const filteredProducts = await Products.find({ "categories.id" : categoryId });
...
exports.getAllProductsByCategoryId = async (req, res) => {
    try{
        ...
        const allowedStaticFilters = ["page", "limit", "sort", "minPrice", "maxPrice", "conditions"];
        ...
        Object.keys(req.query).forEach((key) => {
          if (!allowedStaticFilters.includes(key)) {
              if (typeof req.query[key] === "string") {
                  filter[`details.${key}`] = { $regex: new RegExp(req.query[key], "i") }; // Không phân biệt hoa thường
              } else {
